...
handleStatic path =
  Static.staticApp (Static.defaultFileServerSettings (encodeString posix path))
...
handleEmpty req resp =
  ...
  in resp $
     W.responseLBS
       HTTP.status200
       [("Content-Type", "text/html"), ("Content-Length", l)]
       d
...
handlePong req resp
  ...
  where
    f q =
      let d = maybe "pong" BL.fromStrict (lookup "data" q)
...
handleStatus req resp
  ...
  where
    s
      | (_:code:_) <- Network.Wai.pathInfo req
...
handleStream req resp
  ...
  where
    ...
    f q = do
      let delay =
            maybe (return ()) threadDelay (readMaybeB =<< lookup "delay" q)
      resp $
        W.responseStream HTTP.status200 hdrs $ \write flush ->
  ...
  let handleMessages =
        forever $ do
          ...
          case d of
            ...
            WS.Binary bs -> putStrLn "received binary message"
...
respondWith respond status query contentLength content = do
  ...
  let ct = fromMaybe "text/plain" (lookup "content-type" query)
      hdrs =
        [ ("Content-Type", ct)
        , ( "Content-Length"
          , BL.toStrict . B.toLazyByteString . B.int64Dec $ contentLength)
        ]
...
invalidMethod respond =
  respond $ W.responseLBS HTTP.methodNotAllowed405 [] "Method not allowed"
